...
소프트웨어 공학  
소프트웨어로 제품/ 서비스를 만드는 방법에 대한 핫문  
기획, 제작, 테스트, 출시, 사후관리하는 모든 프로세스  
기획 어떤 걸 만들지 정하는 단계  
개발 기획한 것을 만드는 단계  
...
폭포수 방식(Waterfall)/ 각 단계를 완료하고 다음 단계로 넘어가는 방식  
장점 이해와 관리가 쉬움  
단점 서비스가 복잡할 경우 각 단계를 한 번에 완벽하게 끝내기 힘들 수 있음.   
수정 사항이 많이 생길 경우가 높고, 수정 사항을 바로 반영하기 힘듦.     

애자일(Agile)/ 재빨리 결과물을 만들어 미리 확인하고 수정하며 진행  
중간중간 구현된 서비스를 사용하면서 발전시켜 기능 변경이 쉬움.    
