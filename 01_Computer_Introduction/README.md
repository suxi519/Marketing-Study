...
## 5-3.HTML/CSS    
* 웹 페이지를 만들 때 사용.  
* HTML은 주로 웹 페이지의 **구조와 레이아웃**을 담당, CSS는 웹 페이지의 **시각적인 표현** 담당.      
*  JavaScript를 함께 사용해서, 웹 페이지가 사용자와 **상호 작용**할 수 있게 만들기도 함.  
...
````
## 6-2. 이름 잘 짓기  
전체적인 코드의 흐름 안에서 충분히 구체적(명시적)이고 간결한 이름 짓기  
## 6-3. 스타일 가이드 활용  
언어의 스타일 가이드(규약)에 맞추어 코드를 작성해야 각 잡히고 가독성 좋아짐.  
````python
